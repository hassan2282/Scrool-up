...
function Content() {
    ...
    return (
        <div className='flex flex-col relative w-screen min-h-screen overflow-hidden scroll-smooth'>
            ...
            <div className='h-screen w-screen fixed'>
                <Fade right className="cursor-pointer">
                    <button href='#' onClick={scrollToTop} className={`absolute rounded-full shadow-md text-white shadow-red-900 duration-300 hover:shadow-none cursor-pointer right-10 bottom-10 ${scroll ? '' : 'hidden'}`}><Icon size={48} icon={arrowUpCircle} className='cursor-pointer'/></button>
                </Fade>
            </div>
        </div>
    );
